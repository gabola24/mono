...
async def create_habit(req: HabitCreateRequest, session: AsyncSession = Depends(get_session)):
    habit_id = str(uuid.uuid4())
    now = datetime.now(timezone.utc)
    await session.execute(
        habits.insert().values(
            id=habit_id, name=req.name, frequency=req.frequency, active=True, created_at=now
        )
    )
...
async def list_habits(session: AsyncSession = Depends(get_session)):
    result = await session.execute(
        sa.select(habits).where(habits.c.active == True).order_by(habits.c.created_at)
    )
    return [await _to_response(session, row) for row in result.fetchall()]


@router.post("/habits/{habit_id}/check")
async def check_habit(habit_id: str, session: AsyncSession = Depends(get_session)):
    today = date.today()
    ...
    await session.execute(habit_logs.insert().values(habit_id=habit_id, date=today))
    await session.commit()
    return {"checked": True}


@router.delete("/habits/{habit_id}")
async def delete_habit(habit_id: str, session: AsyncSession = Depends(get_session)):
    await session.execute(
        sa.update(habits).where(habits.c.id == habit_id).values(active=False)
    )
    await session.commit()
