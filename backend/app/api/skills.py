...
async def get_skill_tree(session: AsyncSession = Depends(get_session)):
    nodes_result = await session.execute(
        sa.select(skill_nodes).order_by(skill_nodes.c.xp.desc())
    )
    edges_result = await session.execute(sa.select(skill_edges))
    badges_result = await session.execute(
        sa.select(skill_badges).order_by(skill_badges.c.earned_at.desc())
    )
...
async def get_skill_summary(session: AsyncSession = Depends(get_session)):
    top_result = await session.execute(
        sa.select(skill_nodes).order_by(skill_nodes.c.xp.desc()).limit(5)
    )
    total_result = await session.execute(
        sa.select(sa.func.count()).select_from(skill_nodes)
    )
    badge_count_result = await session.execute(
        sa.select(sa.func.count()).select_from(skill_badges)
    )
...
async def reanalyze_all_references(session: AsyncSession = Depends(get_session)):
    """Re-analyze all references for skill discovery."""
    result = await session.execute(
        sa.select(references.c.content).order_by(references.c.created_at)
    )
    all_badges = []
    for row in result.fetchall():
        badges = await process_skill_discovery(session, row.content)
